...
public class SliderBar extends HBox {
	...
	private static final String SLIDE_STYLE = "-fx-background-color: transparent; " 
			+ "-fx-background-image: url(" + ClassLoader.getSystemResource("images/yellow_panel.png").toString() + "); "
	...
	private void createSlider() {
		...
		slider.setMin(0);
		slider.setMax(100);
		slider.setValue(Main.getVolume()*100);
	...
	private void createLabel() {
		...
		Thread t = new Thread(() -> {
			while (true) {
				try {
					Thread.sleep(50);
				//	System.out.println(Double.parseDouble(number.getText()) / 100);
					Main.setVolume(Double.parseDouble(number.getText()) / 100);
